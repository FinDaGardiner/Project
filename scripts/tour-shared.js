...
function initTour(config) {
  const TOUR_ID = config.id;
  const STORAGE_KEY = `tour:${TOUR_ID}`;
  const tourPoints = config.points || [];

  const ARRIVAL_RADIUS_METERS = config.arrivalRadius ?? 50;
  const ARRIVAL_COOLDOWN_MS = config.arrivalCooldownMs ?? 15000;
  const ROUTE_REDRAW_THRESHOLD_METERS = config.routeRedrawThreshold ?? 12;
  const INITIAL_ZOOM = config.initialZoom ?? 14;
  const USER_CENTER_ZOOM = config.userCenterZoom ?? 16;
  const DEFAULT_CENTER = config.defaultCenter ?? [55.9533, -3.1883];
  const ACCURACY_BUFFER = config.accuracyBuffer ?? 12;
  const MAX_EFFECTIVE_ARRIVAL_RADIUS = config.maxEffectiveArrivalRadius ?? 90;
  ...
  function focusOnUser() {
    ...
    map.setView([pos.lat, pos.lng], Math.max(map.getZoom(), USER_CENTER_ZOOM));
  }
  ...
  function startLocation() {
    ...
    navigator.geolocation.watchPosition(
      (pos) => {
        ...
        if (!hasCentredOnUser) {
          map.setView([lat, lng], USER_CENTER_ZOOM);
        ...
        if (currentIndex < activeRoute.length) {
          ...
          const effectiveArrivalRadius = Math.max(
            ARRIVAL_RADIUS_METERS,
            Math.min(accuracy + ACCURACY_BUFFER, MAX_EFFECTIVE_ARRIVAL_RADIUS)
          );
  ...
  function initMap() {
    map = L.map("map").setView(DEFAULT_CENTER, INITIAL_ZOOM);
